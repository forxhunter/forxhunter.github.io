...
- **Google & XLA:** TPUs rely on **XLA (Accelerated Linear Algebra)**. This compiler acts like a master architect. It analyzes the _entire_ AI model at once and "fuses" operations together into a single, static graph. This requires the developer to be more disciplined (e.g., defining fixed data shapes), but the result is a highly optimized execution plan that squeezes maximum performance from the hardware.
